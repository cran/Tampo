...
function(
   ...
   )
   {
    ...
    if(missing(Size)) {miss<-1
     ...
     if(Condition==2){
       graphics.off()
       cat(paste("WARNING: No tree size met the condition! Please, consider decreasing the 'Cond' argument","\n"))} else{

	if(miss==1) {
       modelOpt<-mvpart(Y~X,data=Data,size=TreeSize,minbucket=Number,minsplit=Number*2,xv="1se")
      } else {
	modelOpt<-mvpart(Y~X,data=Data,size=TreeSize,minbucket=Number,minsplit=Number*2,cp=0.00001)
	}
